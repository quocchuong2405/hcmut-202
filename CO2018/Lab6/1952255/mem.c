...
int mem_init(unsigned int size)
{
  ...
  /* Preallocate the mem pool based on used request */
...
void *mem_alloc(unsigned int size, int opt)
{
...
{
  int found = 0;
  unsigned int worst_spacing = 0;
  struct mem_region *current_region = free_regions, *worst_region = free_regions;
  do
  {
    if (size <= current_region->size)
    {
      int diff = current_region->size - size;
      if (diff > worst_spacing)
      {
        worst_spacing = diff;
        worst_region = current_region;
        found = 1;
      }
    }
    current_region = current_region->next;
  ...
  if (found)
  {
    struct mem_region *tmp =
        (struct mem_region *)malloc(sizeof(struct mem_region));
    tmp->pointer = worst_region->pointer;
    tmp->size = size;
    tmp->next = used_regions;
    tmp->prev = NULL;
    if (used_regions == NULL)
    ...
    if (worst_region->size == size)
    {
      if (worst_region == free_regions)
      {
        free_regions = free_regions->next;
        if (free_regions != NULL)
        {
      ...
      {
        if (worst_region->prev != NULL)
        {
          worst_region->prev->next = worst_region->next;
        }
        if (worst_region->next != NULL)
        {
          worst_region->next->prev = worst_region->prev;
        }
      }
      free(worst_region);
    }
    ...
    {
      worst_region->pointer += size;
      worst_region->size -= size;
    }
    return tmp->pointer;
  }
  else
